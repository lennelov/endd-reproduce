...
from models import vgg, cnn, endd, end, ensemble, cnn_priorNet
...
def train_vgg_end(train_images,
                  ...
                  load_previous_end_dataset=False):
    """Return a trained VGG END model.

    The save_end_dataset and load_previous_end_dataset arguments are useful to avoid having to
    re-create the ensemble predictions.
    ...
    Returns:
        (keras.Model): Trained VGG ENDD model.
    """
    if load_previous_end_dataset:
        with open('train_endd_dataset_rep=2_100.pkl', 'rb') as file:
            train_images, train_ensemble_preds = pickle.load(file)
    else:
        # Get ensemble preds
        train_ensemble_preds = datasets.get_ensemble_preds(ensemble_model, train_images)
    ...
    # loading can make testing much more efficient.
    if save_end_dataset:
        with open('train_endd_dataset_rep=2_100.pkl', 'wb') as file:
            pickle.dump((train_images, train_ensemble_preds), file)
    ...
    base_model = vgg.get_model(dataset_name,
                               ...
                               dropout_rate=dropout_rate,
                               softmax=False)
    end_model = end.get_model(base_model, init_temp=init_temp)

    # Train model
    end_model.fit(data_generator, epochs=n_epochs, callbacks=endd_callbacks)
