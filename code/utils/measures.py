...
def entropy_of_expected(raw, logits):
    ...
    if len(probabilities.shape) == 3:
        ...
        raise ValueError('Logits must be (N_models, N_data_points, N_classes) or (N_data_points, N_classes)')
...
def expected_entropy_pn(logits):
    ...
    return np.sum(-probs * (scipy.special.digamma(alpha + 1) - scipy.special.digamma(alpha_0 + 1)), axis = 1)
